...
10. Versioned remediation rules. `remediation.json` records the ruleset version, changed files, and applied deterministic rule IDs for each run, with a `$schema` pointer and local validation against the v1 remediation metadata contract.
...
- attack-manifest schema validation through `docs/schemas/attack-manifest.schema.json`
...
- versioned remediation metadata through schema-validated `remediation.json`
